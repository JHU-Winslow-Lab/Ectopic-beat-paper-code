...
void StochModel::send_calc_fru_avg()
{
  ...
  double f_A0, f_A2, f_A46, f_A2c, f_A46c, f_A2cc, f_A46cc, f_Allo;
  double NCX_avg_current;
  ...
  for (int i = 0; i < NFRU; i++) {
	NCX_avg_current += FRUs[i].Get_NCXSS_current();
	//NCX_avg_current += FRUs[i].Get_NCXSS_flux();
	...
	f_A0 += FRUs[i].Get_CBD_State_A0();
	f_A2 += FRUs[i].Get_CBD_State_A2();
	f_A46 += FRUs[i].Get_CBD_State_A46();
	f_A2c += FRUs[i].Get_CBD_State_A2c();
	f_A46c += FRUs[i].Get_CBD_State_A46c();
	f_A2cc += FRUs[i].Get_CBD_State_A2cc();
	f_A46cc += FRUs[i].Get_CBD_State_A46cc();
	f_Allo += FRUs[i].Get_Allo();
  ...
  otherstates[index_NCXSSavg] = NCX_avg_current / NFRU;
  otherstates[index_NA0] = f_A0/NFRU;
  otherstates[index_NA2] = f_A2/NFRU;
  otherstates[index_NA46] = f_A46/NFRU;
  otherstates[index_NA2c] = f_A2c/NFRU;
  otherstates[index_NA46c] = f_A46c/NFRU;
  otherstates[index_NA2cc] = f_A2cc/NFRU;
  otherstates[index_NA46cc] = f_A46cc/NFRU;
  otherstates[index_AlloSSavg] = f_Allo/NFRU;
                                ...
                                double *Ito2)
{
  ...
  //Control variables for FRUs
  int ToggleIndicator[2];
  ToggleIndicator[0] = bClampJSR;
  ToggleIndicator[1] = bClampCaSS;
...
#pragma omp parallel for //USE OMP ONLY FOR STOCH MODEL TESTING
  ...
  *Ito2 = NFRU_scale * sum_Ito2;//ERROR!!!
}
