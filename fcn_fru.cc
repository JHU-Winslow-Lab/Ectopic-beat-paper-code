	...
	int ToggleIndicator[Nstates_FRU])
{
	...
	beta_JSR = 1.0 / (1.0 + a1);
	if (ToggleIndicator[index_frustates_CaJSR]) {
		dFRU_states1[index_frustates_CaJSR] = 0;
	}
	else
	{
	...
	if (ToggleIndicator[index_frustates_CaSS]) {
		dFRU_states1[index_frustates_CaSS] = 0;
	}
	else
	{
		dFRU_states1[index_frustates_CaSS] = beta_SS_1 * (JDHPR_1 + JRyR_1 - Jxfer_1 + JNaCa_SS1) / OScale; // dCaSS1
