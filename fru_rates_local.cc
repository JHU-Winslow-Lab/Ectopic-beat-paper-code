	...
	double ryr_kplus_scale)
{
	...
	//const double alpha2 = 0.1*29.9/1e3;//BLOCK THE alpha2 10212017
	...
	// CBD NCX
	double k_a12 = kc12;
	double k_a21 = kc21*(kc32/(kc23*CaSS*1e3+kc32));
	double k_a23 = kc34*(kc23*CaSS*1e3/(kc23*CaSS*1e3+kc32));
	//double k_a23 = 0.1*kc34*(kc23*CaSS*1e3/(kc23*CaSS*1e3+kc32));//BLOCK THE Ka_23 10212017
	double k_a32 = kc43*(kc54/(kc45*CaSS*1e3+kc54));

	int N_A0 = NaCa_state[0];
	int N_A2 = NaCa_state[1];
	int N_A46 = NaCa_state[2];
	...
	// NCX rates
	NaCa_rates[0] = N_A0*k_a12*CaSS*1e3;
	NaCa_rates[1] = N_A2*k_a21;
	NaCa_rates[2] = N_A2*k_a23*CaSS*1e3;
	NaCa_rates[3] = N_A2*alpha2;
	NaCa_rates[4] = N_A46*k_a32;
	NaCa_rates[5] = N_A46*alpha2;
	NaCa_rates[6] = N_A2c*beta2;
	NaCa_rates[7] = N_A2c*k_a23*CaSS*1e3;
	NaCa_rates[8] = N_A2c*alpha4;
	NaCa_rates[9] = N_A46c*beta2;	
	NaCa_rates[10] = N_A46c*k_a32;	
	NaCa_rates[11] = N_A46c*alpha4;
	NaCa_rates[12] = N_A2cc*beta4;
	NaCa_rates[13] = N_A2cc*k_a23*CaSS*1e3;	
	NaCa_rates[14] = N_A46cc*beta4;
	NaCa_rates[15] = N_A46cc*k_a32;
	
	for (int i = 0; i < Nstates_NaCa; i++) {
		rate_sum += NaCa_rates[i];
	}
	...
	*/
	...
	Ito2_rates[0] = ito2_rate_scale * kfIto2 * FRUdep_states[index_frudep_CaSL] * (NIto2_per_cleft - Ito2_state);//ERROR!!!
