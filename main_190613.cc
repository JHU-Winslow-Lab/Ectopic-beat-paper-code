...
int run_experiment(int num, int TotNoData)
{
	...
	s1.Read_DAD_Variables("Two_Exs.txt", num, TotNoData);//change the Cai_default, CaSR_default and ik1_scale
	...
	double t_final = 800;//s1.Get_T_Final(); //60000;
	...
	char fn[80];
	if(num<10){
		strcpy(fn, "v2_2.190613.set0.s000");	
//		strcpy(fn,"test_for_parallel_compute");
	}
	else if(num<100){
		strcpy(fn, "v2_2.190613.set0.s00");
	}
	else if(num<1000){

		strcpy(fn, "v2_2.190613.set0.s0");
	}
	else{
		strcpy(fn, "v2_2.190613.set0.s");
	}
