                     ...
                     const int keepc, const double Jxfer, const double Jtr, const double ICa, const double Ito2, const double Iext, const double JCa)
{
  ...
  std::vector<double> INaCaSM(NFRU,0);	// Na+/Ca++ exchanger current
  ...
  double GKs = 0.0175*k_Gmax_IKs;//0.035; for Winslow's model//0.0112 for Jons's model;  //TODO decreased by mark (Volders); // 0.035	G_Ks in Canine C++ code
  ...
  const double Krb = 1.8; // backward half sat. constant for Ca++ ATPase (mM)ERROR!!!
  ...
  A46 = state[index_A46];
  A2c = state[index_A2c];
  A46c = state[index_A46c];
  A2cc = state[index_A2cc];
  A46cc = 1 - A0 - A2 - A46 - A2c - A46c - A2cc;
  ...
  // CBD12 rates (uM/ms)
  const double kc12 = 4.4548 / 1e3;
  const double kc21 = 0.7542 / 1e3;
  const double kc23 = 1000 / 1e3;
  const double kc32 = 5 / 1e3;
  const double kc34 = 1;
  const double kc43 = 250 / 1e3;
  const double kc45 = 120.9813 / 1e3;
  const double kc54 = 782.1981 / 1e3;
  const double kc56 = 12.2017 / 1e3;
  const double kc65 = 835.3308 / 1e3;
  const double kc67 = 135.4538 / 1e3;
  const double kc76 = 768.8372 / 1e3;

  const double f1 = 0.8;
  const double f2 = 0.1;
  const double alpha2 = 29.9 / 1e3;
  //const double alpha2 = 0.1*29.9 / 1e3;//block alpha2 10212017
  const double beta2 = 5.006 / 1e3;
  const double alpha4 = 0.2929 / 1e3;
  const double beta4 = 0.216 / 1e3;
  ...
  // Make sure that mNa never goes negative
  if (mNa <= 0.0) mNa = 1.e-15;

  INa = GNa * (mNa * mNa * mNa) * hNa * jNa * (V - ENa);

  fKo = pow(Ko / 4.0, 0.5);
  IKr = 1.2 * GKr * fKo * OHerg * (V - EK); //1.2 is the scaling factor, which is the new change.
  // IKs = GKs * (xKs * xKs) * (V - EKs);

  //IKs model Jons et al Sci. Trans. Med. 2011
  IKs = GKs * xKs * (V-EKs);
  ...
  //IKs model Jons et al Sci. Trans. Med. 2011
  xKs_inf = 1.0/(1.0+exp(-((V-IKs_V_shift)-(9.4 + delta_V_half_IKs))/(11.8*k_k_IKs)));

  a1 = 1.0/k_tau_plus_IKs*7.19e-5*(V-IKs_V_shift)/(1+exp(-0.74*(V-IKs_V_shift)));
  a2 = 1.0/k_tau_minus_IKs*0.000786*((V-IKs_V_shift)+40)/(exp(0.103*((V-IKs_V_shift)+40))-1);
  ...
  Jup = 0.75* KSR * (vmaxf * fb - vmaxr * rb) / (1.0 + fb + rb);//0.75 scaling factor is the new change
  //Jup = 1.0 * KSR * (vmaxf * fb - vmaxr * rb) / (1.0 + fb + rb);//0.75 scaling factor is the new change
...
/*
  ...
  */
  ...
  double ncx_numi1 = kr1*kr2*kr3*kr4*kr5*kr6*Cao*(1e3)*(Nai*Nai*Nai)*(1e3*1e3*1e3);
  double ncx_denumi1 = Z1*Nai*Nai*Nai*(1e3*1e3*1e3) + Z3*Nao*Nao*Nao*(1e3*1e3*1e3) + Z4*Cao*(1e3) + Z5*Nai*Nai*Nai*Cao*(1e3*1e3*1e3*1e3) + Z7*Nao*Nao*Nao*Nai*Nai*Nai*(1e3*1e3*1e3*1e3*1e3*1e3);
  // double flux_ncxi;
  const double C_ncx = 500 * 1.534e-4*1e8 / 6.022e23 / 1.8e-11*1e6;
  // double Jncxi;

  // lulu NCX CBD and NCX states derivatives for element
  // double k_a12i;
  // double k_a21i;
  // double k_a23i;
  // double k_a32i;
  ...
  //lulu record for element
  double AlloSMavg = 0;
  double DeltaESMavg = 0;
  double A0SMavg=0;
  double A2SMavg=0;
  double A46SMavg=0;
  double A2cSMavg=0;
  ...
  double t_start, t_end,t_duration;// test the parallel computing power
  //omp_set_num_threads(8);
  t_start = omp_get_wtime();
// #pragma omp parallel for default(shared) reduction(+:JSL,Jxfer_tot,INaCa_tot,INaCaSS,INaCaSMavg,AlloSMavg,DeltaESMavg,A0SMavg,A2SMavg,A46SMavg,A2cSMavg,A46cSMavg,A2ccSMavg)
#pragma omp parallel for default(shared) reduction(+:JSL,Jxfer_tot,INaCa_tot,INaCaSS,INaCaSMavg,AlloSMavg,DeltaESMavg,A0SMavg,A2SMavg,A46SMavg,A2cSMavg,A46cSMavg,A2ccSMavg)
  for (int i = 0; i < NFRU; i++) {

    int tmppp = omp_get_thread_num();
    // printf("%d\n",tmppp);
  	//lulu allo 
  	double A0i;
  	double A2i;
  	...
  	double flux_ncxi;
  	double Jncxi;
  	// int hahaha = omp_get_num_threads();
    ...
    if (ci < 1e-9) { 
	 ...
	 //lulu allo
	  A0i = state[N_states + NFRU+i];
	  A2i = state[N_states + 2*NFRU + i];
	  A46i = state[N_states + 3*NFRU + i];
	  A2ci = state[N_states + 4*NFRU + i];
	  A46ci = state[N_states + 5*NFRU + i];
	  A2cci = state[N_states + 6*NFRU + i];
	  A46cci = 1 - A0i - A2i - A46i - A2ci - A46ci - A2cci;
	  Alloi = f1*f2*A2ci + f1*A46ci + f2*A2cci + A46cci;
  	//Alloi = 0.0;//ERROR!!! FOR 08302017

	  //lulu electrochemical
	  double ncx_numi2 = kf1*kf2*kf3*kf4*kf5*kf6*ci*(1e3)*(Nao*Nao*Nao)*(1e3*1e3*1e3);
	  double ncx_denumi2 = Z2*ci*(1e3) + Z6*Nao*Nao*Nao*ci*(1e3*1e3*1e3*1e3) + Z8*ci*Cao*(1e3*1e3);
	  ...
	  INaCaSMavg += INaCaSM[i];
	  AlloSMavg += Alloi;
	  DeltaESMavg += flux_ncxi*C_ncx;
	  A0SMavg += A0i;
	  A2SMavg += A2i;
	  A46SMavg += A46i;
	  A2cSMavg += A2ci;
    ...
    //INaCaSM[i] = ncx_scale * frac_ncx_sl * kNaCa * a5_ncx * (a1_ncx - a2_ncx) / (a4_ncx * a3_ncx);
	...
	if (bClampCaPD) {
		...
		Fstate[N_states + i] = beta_i_i*(jxfer_i*(VSS / VSL) + jsl2cyto + Jdiff + INaCaSM[i] * a1); //Assign Fstate within omp loop for speed
	}
	...
	// lulu NCX CBD and NCX states derivatives for element
	k_a12i = kc12;
	k_a21i = kc21*(kc32 / (kc23*ci*1e3 + kc32));
	k_a23i = kc34*(kc23*ci*1e3 / (kc23*ci*1e3 + kc32));
  	//k_a23i = 0.1*kc34*(kc23*ci*1e3 / (kc23*ci*1e3 + kc32));//BLOCK THE Ka23 10212017
	k_a32i = kc43*(kc54 / (kc45*ci*1e3 + kc54));
	
	// CBD12 state (ODEs) for element
	Fstate[N_states + NFRU + i] = -k_a12i*ci*1e3*A0i + k_a21i*A2i;
	Fstate[N_states + 2 * NFRU + i] = k_a12i*ci*(1e3)*A0i - k_a21i*A2i - k_a23i*ci*(1e3)*A2i + k_a32i*A46i - alpha2*A2i + beta2*A2ci;
	Fstate[N_states + 3 * NFRU + i] = k_a23i*ci*(1e3)*A2i - k_a32i*A46i - alpha2*A46i + beta2*A46ci;
	Fstate[N_states + 4 * NFRU + i] = alpha2*A2i - beta2*A2ci - k_a23i*ci*(1e3)*A2ci + k_a32i*A46ci - alpha4*A2ci + beta4*A2cci;
	Fstate[N_states + 5 * NFRU + i] = k_a23i*ci*(1e3)*A2ci - k_a32i*A46ci + alpha2*A46i - beta2*A46ci - alpha4*A46ci + beta4*A46cci;
	Fstate[N_states + 6 * NFRU + i] = -k_a23i*ci*(1e3)*A2cci + k_a32i*A46cci + alpha4*A2ci - beta4*A2cci;
  }
  t_end = omp_get_wtime();
  ...
  double k_a12 = kc12;
  double k_a21 = kc21*(kc32 / (kc23*Cai*1e3 + kc32));
  double k_a23 = kc34*(kc23*Cai*1e3 / (kc23*Cai*1e3 + kc32));
  //double k_a23 = 0.1*kc34*(kc23*Cai*1e3 / (kc23*Cai*1e3 + kc32));//BLOCK THE Ka_23 10212017
  double k_a32 = kc43*(kc54 / (kc45*Cai*1e3 + kc54));

  // CBD12 state (ODEs)
  dA0 = -k_a12*Cai*1e3*A0 + k_a21*A2;
  dA2 = k_a12*Cai*(1e3)*A0 - k_a21*A2 - k_a23*Cai*(1e3)*A2 + k_a32*A46 - alpha2*A2 + beta2*A2c;
  dA46 = k_a23*Cai*(1e3)*A2 - k_a32*A46 - alpha2*A46 + beta2*A46c;
  dA2c = alpha2*A2 - beta2*A2c - k_a23*Cai*(1e3)*A2c + k_a32*A46c - alpha4*A2c + beta4*A2cc;
  dA46c = k_a23*Cai*(1e3)*A2c - k_a32*A46c + alpha2*A46 - beta2*A46c - alpha4*A46c + beta4*A46cc;
  dA2cc = -k_a23*Cai*(1e3)*A2cc + k_a32*A46cc + alpha4*A2c - beta4*A2cc;

  //double INaCa_cyto = ncx_scale * (1.0-frac_ncx_sl) * kNaCa * a5 * (a1 - a2) / (a4 * a3);//origin version
  ...
  if (bClampCai) {
    ...
    dCai = beta_i * ( -JSL / Vmyo - Jup - Jtrpn - (ICab + IpCa - JCa - 2.0*INaCa_cyto) * 0.5 * a1); //2 times of INaCa_cyto
  ...
  */
  ...
  // a2 = IK1 + IKp + INaCa_tot + INaK + Ito1;//20190419
  a3 = IpCa + ICab + INab;
  ...
  Fstate[index_A0] = dA0;
  Fstate[index_A2] = dA2;
  Fstate[index_A46] = dA46;
  Fstate[index_A2c] = dA2c;
  Fstate[index_A46c] = dA46c;
  Fstate[index_A2cc] = dA2cc;
  ...
  if (keepc) {
	...
	//lulu
	current[index_AlloCyto] = Allo;
	current[index_DeltaECyto] = flux_ncx*C_ncx;
	current[index_AlloSMavg] = AlloSMavg;
	current[index_DeltaESMavg] = DeltaESMavg;

	current[index_A0SMavg] = A0SMavg;
	current[index_A2SMavg] = A2SMavg;
	current[index_A46SMavg] = A46SMavg;
	current[index_A2cSMavg] = A2cSMavg;
	current[index_A46cSMavg] = A46cSMavg;
	current[index_A2ccSMavg] = A2ccSMavg;
	current[index_INaCaCyto] = INaCa_cyto;
	current[index_INaCaSMavg] = INaCaSMavg;
  }
