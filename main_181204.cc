...
int main(int argc, char **argv)
{
	...
	// }
	// s1.Set_IKr_beta_ikr_rate_scale(4);
	        int TotNoData=90;
	for (int Repeat = 0; Repeat< 100;++Repeat){
		for (int LQTS1_index = TotNoData; LQTS1_index < TotNoData+5; ++LQTS1_index) { //original stop is 3
			s1.Load_State_File("v2_2.180713.set2.s01.StochData.txt", "v2_2.180713.set2.s01.FRUsData.txt");
			s1.Read_LQTS1_p("EAD_Rand_samples.txt", LQTS1_index,200);
			...
			char fn[80];
			if (LQTS1_index < 10){
				strcpy(fn, "v2_2.181204.set0.00");
			}
			else if(LQTS1_index < 100){
				strcpy(fn, "v2_2.181204.set0.0");
			}
			else
			{
				strcpy(fn, "v2_2.181204.set0.");
			}
