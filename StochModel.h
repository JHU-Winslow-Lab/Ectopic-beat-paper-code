...
public:
  ...
  void Load_State_File(std::string filenameStoch, std::string filenameFRUs);
  ...
  //For constrain Ca removal calculation
  //void Set_serca_vmax_scale(double v);
  ...
  void Write_GridNCXPD(std::ostream &os, double t);
  void Write_GridNCXD(std::ostream &os, double t);
  void Write_GridCaD(std::ostream &os, double t);
  void Write_GridJRyR(std::ostream &os, double t);
  void Write_GridRyROpen(std::ostream &os, double t);

  //Write Release unit samples
  void Write_Sample_Header(std::ostream &os);
  void Write_Sample_Data(std::ostream &os, double t, int i); //Writes time and all variables
  ...
  //Stoch Data Output
  void Write_Stoch_Data(std::ostream &os, double t);
  void Write_FRU_Data(std::ostream &os, double t);
  ...
  int Toggle_Clamp_Cai();
  int Toggle_Clamp_JSR();
  int Toggle_Clamp_CaSS();
  int Toggle_Clamp_CaPD();


  //Alter scaling factor function
  double Set_IKr_scale(double c);
  double Set_IKs_scale(double c);
  void Set_mode2_Frac(double c);
  void Set_IKr_beta_ikr_rate_scale(double c);

  //Change the CaNSR level (study)
  void Set_CaNSR(double c);
  void Set_Cai2(double c);

  //Sensitivity analysis
  void Set_k_tau_plus_IKs(double c);
  void Set_k_tau_minus_IKs(double c);
  void Set_k_k_IKs(double c);
  void Set_k_Gmax_IKs(double c);
  void Set_delta_V_half_IKs(double c);

  void set_PCa();
  double* get_data();
  //Set extracellular concentration functions
  void Set_ExIonsLevel(const double UCao, const double UNao, const double UKo, const double UClo);

  //Read LQTS1 mutation parameter
  void Read_LQTS1_p(std::string LQTS1_Data, int num, int TotNoData);
  void Read_LQTS1_Rand(std::string LQTS1_Data_Rand, int num);
  void Read_TdP_Drug_Rand(std::string TdP_Drug_Data_Rand, int num);
  void Read_AP_p_Rand(std::string AP_p_Rand, int num);
  void Read_DAD_Variables(std::string DAD_data, int num, int TotNoData);
...
private:
  ...
  double ikr_scale;
  double iks_scale;// new add
  double ina_scale;// new add
  ...
  //////////////////////////////
  //Parameters For LQTS1
  //////////////////////////////
  double k_tau_plus_IKs;
  double k_tau_minus_IKs;
  double k_k_IKs;
  double k_Gmax_IKs;
  double delta_V_half_IKs;

  //////////////////////////////
  //Parameters For TdP Drug
  //////////////////////////////
  double k_IKr;
  double k_INa;
  double k_ICa;
  ...
  //Ion concentrations
  static double Nao; // extracellular Na+ concentration (mM)
  static double Ko; // extracellular K+ concentration (mM)
  static const double Default_Ki;

  //Beta-Adrenergic conditions
  static const double beta_iks_scale;
  static double beta_ikr_rate_scale;
  ...
  static double Clo; // extracellular Cl- concentration (mM)
