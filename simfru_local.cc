			...
			int ToggleIndicator[Nstates_FRU])
{
  ...
  while (!bDone) {
	  ...
	  double Ca_old = FRU_states[1];

    //Sum of residuals at beginning of time step
    Ri0 = Ri;

    R_sum = fru_rates(FRUdep_states, LCC_rates, LCC_index, LCC_Vdep_rates, RyR_trans_rates, Ito2_rates, NaCa_rates, beta_flag, ryr_kplus_scale);
    ...
    if (Ri > 0) {
      ...
      //fire reaction
      fru_fire_rxn(runif * R_sum, LCC_rates, LCC_index, LCC_Vdep_rates, RyR_trans_rates, Ito2_rates, NaCa_rates);
